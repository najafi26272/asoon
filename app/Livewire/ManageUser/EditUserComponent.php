...
class EditUserComponent extends Component
{
    ...
    public function updated(){
        if ($this->nameImage) {
            $folderDate = sprintf('%s-%s', Carbon::now()->year, Carbon::now()->month);
            $path = $this->newImage->store('public/uploads/organization/' . $folderDate);
            $filename = explode('/', $path);

            $user = User::find($this->userId)->update([
                'avatar' => $folderDate . '/' . $filename
            ]);
        }
    }
    ...
    {
        return view('livewire.manage-user.edit-user-component');
    }
}
